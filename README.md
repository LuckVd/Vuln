...
基于 Umi + React + Ant Design 的漏洞管理系统，**纯Mock模式**，提供完整的漏洞生命周期管理和审批流程功能，无需数据库配置即可运行。
...
项目采用**纯Mock模式**，包含完整的 Mock 数据和 API，支持：
...
### Mock 特性
- **数据持久化**: Mock数据在内存中保持状态，支持CRUD操作
- **动态关联**: 漏洞和审批单之间可以建立和解除关联
- **实时同步**: 数据操作会实时反映在相关页面上
- **丰富的测试数据**: 包含多种风险等级、来源和状态的漏洞

Mock 数据位于 `mock/` 目录：
- `mock/vuln.ts`: 漏洞相关 Mock 数据和 API
- `mock/approval.ts`: 审批相关 Mock 数据和 API
- `mock/index.ts`: Mock API 统一配置
...
1. **纯Mock模式**: 本项目使用Mock数据，无需数据库配置，开箱即用
2. 数据在内存中保持，重启服务后会重置为初始状态
3. 路由参数验证和错误处理可以根据实际需求进一步完善
4. 权限控制需要在实际项目中实现
5. 实际部署时需要替换为真实的 API 和数据库服务
